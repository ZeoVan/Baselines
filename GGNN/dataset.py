...
class MethodNamePredictionData():
    ...
    def __init__(self, opt, data_path, is_training=True, is_testing=False, is_validating=False, live_test=False):
        ...
        if os.path.exists(saved_input_filename):
            ...
            all_data_node_id, all_data_node_type, all_data_node_token = self.load_program_graphs_from_directory(
                data_path)
    ...
    def load_program_graphs_from_directory(self, directory):
    ...
    def pad_batch(self, initial_node_representations, max_node):
        ...
        for i in range(0, len(initial_node_representations)):
            ...
            initial_node_representations[i] = np.concatenate(
                (initial_node_representations[i], np.zeros([remaining, 30])), axis=0)
        ...
        for i in range(0, len(node_token_indices)):
            ...
            if remaining_nodes > 0:
                if remaining_sub_tokens == 0:
                    # print("case 1....")
                    node_token_indices[i] = np.concatenate(
                        (node_token_indices[i], np.zeros((remaining_nodes, num_sub_tokens_of_batch))), axis=0)
                else:
                    # print("case 2....")
                    node_token_indices[i] = np.concatenate(
                        (node_token_indices[i], np.zeros((node_token_indices[i].shape[0], remaining_sub_tokens))),
                        axis=1)
                    node_token_indices[i] = np.concatenate(
                        (node_token_indices[i], np.zeros((remaining_nodes, num_sub_tokens_of_batch))), axis=0)
            else:
    ...
    def make_batch(self, elements):
        ...
        batch_data['node_token_indices'] = self.pad_node_tokens(batch_data['node_token_indices'], num_nodes_of_batch,
                                                                num_sub_tokens_of_batch)
    ...
    def make_minibatch_iterator(self):
        ...
        if self.is_training:
            ...
            for bucket_idx, buckets_data in buckets.items():
                ...
                buckets_to_process[bucket_idx] = random.sample(buckets_data,
                                                               int(len(buckets_data) / self.sampling_size))
